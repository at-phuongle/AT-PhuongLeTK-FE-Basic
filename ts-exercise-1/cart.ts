var $box: HTMLElement = document.getElementById('js-cart-list');
var $total: HTMLElement = document.getElementById('js-total');
var j: number;
function showCart(): void {
  $box.innerHTML = '';
  var lengthListCart = listCart.length;
  for (var i: number = 0; i < lengthListCart; i++) {
    j = i + 1;
    $box.innerHTML +=
      '<tr id="js-remove' + j + '" >' +
...
function sumTotal(): number {
  var sum: number = 0;
  for (var i: number = 0; i < listCart.length; i++) {
...
function deleteItem(): void {
  var dele: HTMLCollection= document.getElementsByClassName('js-bt-delete');
